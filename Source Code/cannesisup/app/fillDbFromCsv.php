...
if (fopen($urlCSV, "r")) {
    ...
    for ($i = 0; $i < $count-3; $i++) {
	    ...
	    if ($i > 0) { // pour éviter la ligne des titres
	        ...
	        $email = StringManager::cleaner($participant[13]);
	        $dodo = StringManager::cleaner($participant[17]);
	        $entreprise = StringManager::cleaner($participant[24]);
	        $lunchPack = StringManager::cleaner($participant[27]);
	        $participantChoicesFromCsv = [26, 31, 32, 33, 36];
